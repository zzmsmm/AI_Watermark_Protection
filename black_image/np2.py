...
w1 = cv2.imread('./w1_28.png', cv2.IMREAD_GRAYSCALE) # 使用 opencv读取图像
# img = cv2.resize('./w1_28.png', (28,28)) # 图
# print(img.shape)   # numpy数组格式为（H,W,C）

transf = transforms.ToTensor()
w1 = transf(w1)  # tensor数据格式是torch(C,H,W)
w2 = cv2.imread('./w2_28.png', cv2.IMREAD_GRAYSCALE) # 使用 opencv读取图像
w2 = transf(w2)  # tensor数据格式是torch(C,H,W)
w1=w1.resize_(784,1)
w2=w2.resize_(784,1)
...
for epoch in range(epoch1):
    ...
    for step,(b_x,b_y) in enumerate(poison_loader):
        ...
        op=cnn(b_x)
        loss=loss_function(op,b_y.squeeze())
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
    error_count=0
